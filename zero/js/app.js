...
const Zero = (() => {
  ...
  /* ============================================================
     NATIVE CORE (C++) — the fast offline brain, reached through
     the Python bridge (JS → Python → C++). Ranks your own notes
     and memories with real TF-IDF, no model and no network beyond
     your own machine.
     ============================================================ */
  let nativeReady = null;   // null unknown · true/false once checked

  // Where the bridge lives: the page's own origin when served over http,
  // else the engine address you set (the bundled Python server).
  function coreBase() {
    if (location.protocol.startsWith('http')) return location.origin;
    const u = store.raw(LS.engineUrl);
    return (u && /^https?:/.test(u)) ? u.replace(/\/+$/, '') : 'http://localhost:8080';
  }

  async function nativePing() {
    try {
      const r = await guardedFetch(coreBase() + '/api/core/ping', {}, 'ai');
      const d = await r.json();
      nativeReady = !!d.available;
    } catch { nativeReady = false; }
    const pill = document.getElementById('nativePill');
    if (pill) pill.textContent = nativeReady ? 'C++ core online' : 'C++ core off';
    return nativeReady;
  }

  async function nativeCore(mode, query) {
    const docs = [
      ...memories.map(m => ({ title: 'memory', text: m.text })),
      ...notes.map(n => ({ title: 'note', text: n.text })),
    ];
    const r = await guardedFetch(coreBase() + '/api/core', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ mode, query, docs }),
    }, 'ai');
    return r.json();
  }

  /* `recall <query>` — search your own notes+memories with the C++ core. */
  async function recall(query) {
    if (!notes.length && !memories.length) return 'Nothing to recall yet — add notes or tell me to remember things first.';
    try {
      const ans = await nativeCore('answer', query);
      if (ans.available === false) {
        return 'The native C++ core is not running. Start Zero with the Python server and build it: run build-core.sh (or build-core.bat), then restart the server. ' +
               'Until then I search locally in JavaScript instead.';
      }
      const hits = await nativeCore('search', query);
      let out = '';
      if (ans.answer && ans.answer.trim()) out += ans.answer.trim() + '\n\n';
      if (hits.results && hits.results.length) {
        out += 'From your notes (ranked by the C++ core):\n' +
          hits.results.slice(0, 5).map((h, i) => `  ${i + 1}. [${h.title}] ${h.text}`).join('\n');
      } else if (!out) {
        out = `Nothing in your notes matched "${query}".`;
      }
      return out.trim();
    } catch (e) {
      // Bridge not up: fall back to a plain JS scan so recall still works.
      const q = query.toLowerCase();
      const hit = [...memories, ...notes].filter(x => x.text.toLowerCase().includes(q));
      return hit.length
  ...
  function offlineCommand(q) {
    ...
    if (rec) {
      const holder = bubble('Searching your notes with the C++ core…', 'zero');
      recall(rec[1].replace(/[.?!]$/, '').trim()).then(t => { holder.textContent = t; }).catch(() => {});
      return HANDLED;
    }
    if (low === 'help') {
      return [
        ...
        '  recall <text>       search your notes/memories with the C++ core',
        '  map <place>         open any place on the world map',
        '',
  ...
  function init() {
    ...
    nativePing();               // detect the C++ core through the bridge
